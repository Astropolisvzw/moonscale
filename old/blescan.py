...
def parse_events(sock, loop_count=100):
    ...
    for i in range(0, loop_count):
        ...
        if event == bluez.EVT_INQUIRY_RESULT_WITH_RSSI:
            ...
            #print(f"meta event: {pkt[3]}, {type(pkt[3])}")
            subevent = pkt[3]
            pkt = pkt[4:]
            if subevent == EVT_LE_CONN_COMPLETE:
                ...
                #print(f"ADVERTISING: {pkt[0]}")
                num_reports = pkt[0]
                report_pkt_offset = 0
                for i in range(0, num_reports):

                    if (DEBUG == True) and packed_bdaddr_to_string(pkt[report_pkt_offset + 3:report_pkt_offset + 9]) == 'f4:ad:91:94:ab:36':
                        print("-------------")
